...
exports.getpartnerRq= async (req, res) => {
    try {
        ...
        const [staffResult] = await db.promise().query('SELECT company_name ,date, FROM become_a_partner WHERE becomestatus_id = 1 ORDERBY  date');
        

        // Return the object with the counts
        res.status(200).json(staffResult);
